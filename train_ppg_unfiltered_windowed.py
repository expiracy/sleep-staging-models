"""
Train Window-Adaptive PPG + Unfiltered PPG Model
Based on train_windowed_crossattn.py structure
"""
import torch
...
from models.ppg_unfiltered_crossattn_windowed import PPGUnfilteredWindowedCrossAttention
from windowed_dataset import get_windowed_dataloaders
from gpu_utils import setup_gpu_memory_limit, print_gpu_memory_usage


class PPGUnfilteredWindowedTrainer:
    def __init__(self, config, run_id=None):
        ...
        config['model_type'] = 'ppg_unfiltered_windowed'
    ...
    def setup_directories(self):
        ...
        model_name = f"ppg_unfiltered_windowed_{timestamp}"
    ...
    def train_epoch(self, model, dataloader, optimizer, criterion, scheduler, epoch):
        ...
        for batch_idx, (ppg, ecg, labels) in enumerate(pbar):
            # Only use PPG for this model (ignore ECG)
            ...
            if self.use_amp:
                with autocast():
                    outputs = model(ppg)  # Model only takes PPG input
    ...
    def train(self):
        """Main training loop"""
        print(f"\n{'=' * 60}")
        print("Training Window-Adaptive PPG + Unfiltered PPG Model")
        print(f"{'=' * 60}")

        # Prepare data
        data_paths = {
            'ppg': self.config['data']['ppg_file'],
            'real_ecg': self.config['data']['ecg_file'],  # Still needed for dataloader
        ...
        train_loader, val_loader, test_loader, train_dataset, val_dataset, test_dataset = get_windowed_dataloaders(
            ...
            model_type='multimodal',  # Still multimodal dataloader
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train PPG Unfiltered Windowed Cross-Attention Model')
    ...
    for run in range(1, n_runs + 1):
        ...
        # Train
        trainer = PPGUnfilteredWindowedTrainer(config, run_id=run)
    ...
    if n_runs > 1:
        ...
        summary_results = {
            'num_runs': n_runs,
            'model_type': 'ppg_unfiltered_windowed',
            'overall_metrics': {
                'accuracy': {
        ...
        summary_path = os.path.join(config['output']['save_dir'],
                                    f'ppg_unfiltered_windowed_summary_{timestamp}.json')
